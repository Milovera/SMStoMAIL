...
class FiltersDaoTest {
    private lateinit var dao: IFilterDao
    ...
    fun repositoryInsertDuplicate_replaceItemInDao() = runTest {
        addOneItemToDao()
        val newItem = item1.copy(type = Filter.Type.MessageExclude, value = "newValue")
        dao.insert(newItem)
        val allItems =  dao.getAllItems()
    ...
    fun repositoryUpdateItem_updateItemInDao() = runTest {
        addOneItemToDao()
        val newItem = item1.copy(type = Filter.Type.MessageInclude, value = "updatedValue")
        dao.update(newItem)
        val allItems =  dao.getAllItems()
        assert(allItems.first() == newItem)
    }
